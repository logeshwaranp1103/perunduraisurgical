...
import { WHATSAPP_NUMBER } from '../data/productsData';
...
export default function ProductModal({ product, onClose }) {
  ...
  const waText = `Hello, I'm interested in ${product.name}. Please share the price and availability.`;
  const waUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(waText)}`;

  return (
    <div
      className="pm-overlay active"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="product-modal" onClick={(e) => e.stopPropagation()}>
        {/* Close button */}
        <button className="pm-close" onClick={onClose} aria-label="Close product details">
